// GET /api/stop-and-shop?recipe=<slug>            → Stop & Shop list page (single recipe)
// GET /api/stop-and-shop?menu=today               → Stop & Shop list page (full menu)
// GET /api/stop-and-shop?menu=today&date=YYYY-MM-DD → list page for that ET day
// GET /api/stop-and-shop?menu=today&format=json   → JSON
// GET /api/stop-and-shop?menu=today&redirect=bulk → 302 to bulk search URL
//
// Mirrors api/amazon-cart.js and api/walmart.js — standalone endpoint so
// the daily email and external integrations can deep-link Stop & Shop
// without going through /api/grocery?service=stopandshop.
//
// Stop & Shop doesn't expose a public cart-add API, so the page surfaces:
//   • one big "Search all on Stop & Shop" button (their multi-token search)
...
function renderListPage(title, ingredients, bulkUrl, site){
  const rows = ingredients.map(it => {
    ...
    const fd = stopAndShopItemSearchURL(it.name);
    return `<li>
      <label><input type="checkbox" checked>
        <span class="qty">${escapeHtml(qty)}</span>
        <span class="name">${escapeHtml(it.name)}</span>
      </label>
      <a class="find" href="${fd}" target="_blank" rel="noopener" title="Find on Stop & Shop">Find →</a>
    </li>`;
  }).join('');

  return `<!DOCTYPE html><html lang="en"><head>
<meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>${escapeHtml(title)} — Stop & Shop · Dinner Tonight</title>
<style>
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .eyebrow{font-size:11px;letter-spacing:.34em;text-transform:uppercase;color:#e21a23;margin-bottom:8px}
  ...
  .bulk{display:block;background:#e21a23;color:#0a0a0a;text-decoration:none;padding:16px;border-radius:14px;font-size:16px;font-weight:bold;text-align:center;margin-bottom:18px}
  ...
  .name{font-size:15px}
  .find{font-size:13px;color:#e21a23;text-decoration:none;border:1px solid #e21a23;padding:4px 10px;border-radius:6px}
  ...
  <div class="eyebrow">Stop & Shop</div>
  <h1>${escapeHtml(title)}</h1>
  <div class="sub">Pantry staples excluded. Tap each "Find →" to drop into a single-item search — multi-item search silently truncates on Stop & Shop.</div>
  <a class="bulk" href="${bulkUrl}" target="_blank" rel="noopener">🛒 Open Stop & Shop search for all items</a>
  <ul>${rows}</ul>
  <div class="note">Stop & Shop doesn't expose a public cart-add API, so this page is search-driven. Signed-in shoppers can add each item with one tap.</div>
</div></body></html>`;
}
...
export default async function handler(req, res){
  try {
    ...
    const bulkUrl = stopAndShopSearchURL(ingredients);
    ...
    if (format === 'json'){
      ...
      return res.status(200).json({
        ...
        items: ingredients.map(it => ({
          name: it.name,
          url: stopAndShopItemSearchURL(it.name)
        }))
      });
    }
