// GET /api/freshdirect?recipe=<slug>            → FreshDirect list page (single recipe)
// GET /api/freshdirect?menu=today               → FreshDirect list page (full menu)
// GET /api/freshdirect?menu=today&date=YYYY-MM-DD → list page for that ET day
// GET /api/freshdirect?menu=today&format=json   → JSON
// GET /api/freshdirect?menu=today&redirect=bulk → 302 to bulk search URL
//
// Mirrors api/amazon-cart.js and api/walmart.js — standalone endpoint so
// the daily email and external integrations can deep-link FreshDirect
// without going through /api/grocery?service=freshdirect.
//
// FreshDirect doesn't expose a public cart-add API, so the page surfaces:
//   • one big "Search all on FreshDirect" button (their multi-token search)
...
function renderListPage(title, ingredients, bulkUrl, site){
  const rows = ingredients.map(it => {
    ...
    return `<li>
      ...
      <a class="find" href="${fd}" target="_blank" rel="noopener" title="Find on FreshDirect">Find →</a>
    </li>`;
  }).join('');

  return `<!DOCTYPE html><html lang="en"><head>
<meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>${escapeHtml(title)} — FreshDirect · Dinner Tonight</title>
<style>
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .eyebrow{font-size:11px;letter-spacing:.34em;text-transform:uppercase;color:#7cc24a;margin-bottom:8px}
  ...
  .bulk{display:block;background:#7cc24a;color:#0a0a0a;text-decoration:none;padding:16px;border-radius:14px;font-size:16px;font-weight:bold;text-align:center;margin-bottom:18px}
  ...
  .name{font-size:15px}
  .find{font-size:13px;color:#7cc24a;text-decoration:none;border:1px solid #7cc24a;padding:4px 10px;border-radius:6px}
  ...
  <div class="eyebrow">FreshDirect</div>
  <h1>${escapeHtml(title)}</h1>
  <div class="sub">Pantry staples excluded. Tap each "Find →" to drop into a single-item search — multi-item search silently truncates on FreshDirect.</div>
  <a class="bulk" href="${bulkUrl}" target="_blank" rel="noopener">🥬 Open FreshDirect search for all items</a>
  <ul>${rows}</ul>
  <div class="note">FreshDirect doesn't expose a public cart-add API, so this page is search-driven. Signed-in shoppers can add each item with one tap.</div>
</div></body></html>`;
}
...
export default async function handler(req, res){
  try {
    ...
    const bulkUrl = freshDirectSearchURL(ingredients);
    ...
    if (format === 'json'){
      ...
      return res.status(200).json({
        ...
        items: ingredients.map(it => ({
          name: it.name,
          url: freshDirectItemSearchURL(it.name)
        }))
      });
    }
